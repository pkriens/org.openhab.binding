...
@Component
public class VitotronicBridgeDiscovery extends AbstractDiscoveryService  implements org.eclipse.smarthome.config.discovery.DiscoveryService{
		...
		public void run() {
			...
			try {
				...
				if (receiveMessage.startsWith(VitotronicBindingConstants.BROADCAST_MESSAGE)) {
					...
					String adapterID = receiveMessage.substring(VitotronicBindingConstants.BROADCAST_MESSAGE.
							length()).toUpperCase();
